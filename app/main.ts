...
const server = net.createServer(
  (socket: net.Socket & { socketId?: number }) => {
    ...
    socket.on("data", (data) => {
      ...
      commands.forEach(async (command) => {
        ...
        if (commandName === "PING") return socket.write(Base.ping());
        ...
        if (commandName === "RPUSH") {
          ...
          return socket.write(await List.rpush({ key, value }));
        }
        ...
        if (commandName === "BLPOP") {
          ...
          List.blpop({ key, timeout }).then((res) => socket.write(res));
          return;
        }
